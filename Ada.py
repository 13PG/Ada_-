...
def adaBoostTrainDS(dataArr, classLabels, numIt=40):  #迭代次数是算法中唯一需要用户指定的参数（不给的话就是默认40 次）
    ...
    for i in range(numIt):  #numIt次迭代
        ...
        weakClassArr.append(bestStump)   #保存当前弱分类器的所有信息到列表中
        ...
        csl=np.log(error_times)     #好的，那现在问题来了，为什么是用ln,而不是别的函数呢？
        csl[csl==0]=1   #防止分母为0的情况出现  
...
if __name__ == "__main__":
    dataMat, classLabels = loadDataSet('final_data_train.csv')   #导数据
    classifierArr = adaBoostTrainDS(dataMat, classLabels, 80)

    #测试公式的可靠性（已经确定还OK了，就直接注掉它了）
    dataMat, classLabels = loadDataSet('final_data_text.csv')   #导数据
